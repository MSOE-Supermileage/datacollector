...
def get_accessory():
    ...
    print('VID: 0x%0.4x - PID: 0x%0.4x' % (VID_ANDROID_ACCESSORY, PID_ANDROID_ACCESSORY))
    
    dev = usb.core.find(idVendor = VID_ANDROID_ACCESSORY, idProduct = PID_ANDROID_ACCESSORY)
...
def get_android_device():
    ...
    android_dev = usb.core.find(idVendor = VID_GALAXY_NEXUS_DEBUG, idProduct = PID_GALAXY_NEXUS_DEBUG)
...
def set_strings(ldev):
    ...
    send_string(ldev, 4, 'http://zombiebrainzjuice.fr/py-android-accessory/')
    send_string(ldev, 5, '2254711SerialNo.')
    
    print("Successfully sent strings")
